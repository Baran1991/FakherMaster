...
{
    [Serializable]
    public partial class PersianDate : IConvertible, IComparable<PersianDate>
    {
        static PersianDate()
        {
//            rSentinel.Check();
        ...
        {
            DayOfWeek dayOfWeek = Calendar.GetDayOfWeek(ToSystemDateTime());
            int i = (int)dayOfWeek;
            WeekDay d = (WeekDay)( (i + 1) % typeof(WeekDay).GetEnumItems().Length );
            return d;
        }
...
//        }
        public static implicit operator PersianDate(string date)
        {
            return FromString(date);
        }
        ...
        public static PersianDate FromString(string dateString)
        {
            try
            {
                ...
                int yearPrefix = Today.Year - (Today.Year % 100);

                string[] items = dateString.Split(' ');
                string[] dateItems = items[0].Split('/');

                int first = Convert.ToInt32(dateItems[0]);
                int second = Convert.ToInt32(dateItems[1]);
                int third = Convert.ToInt32(dateItems[2]);
                ...
                try
                {
                    if(first.ToString().Length == 4)
                        persianDate.Year = first;
                    else
                        persianDate.Year = yearPrefix + first;

                    persianDate.Month = second;
                    persianDate.Day = third;

                    systemDateTime = persianDate.ToSystemDateTime();
                }
                catch (Exception)
                {
                    if (third.ToString().Length == 4)
                        persianDate.Year = third;
                    else
                        persianDate.Year = yearPrefix + third;

                    persianDate.Month = second;
                    persianDate.Day = first;
                }
...
//            throw new NotImplementedException();
    ...
    public enum WeekDay
    {
        [EnumDescription("شنبه")]
        Saturday,
        [EnumDescription("یک شنبه")]
        Sunday,
        [EnumDescription("دو شنبه")]
        Monday,
        [EnumDescription("سه شنبه")]
        Tuesday,
        [EnumDescription("چهار شنبه")]
        Wednesday,
        [EnumDescription("پنج شنبه")]
        Thursday,
        [EnumDescription("جمعه")]
        Friday
    }
}