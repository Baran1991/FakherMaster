...
namespace Fakher.UI
{
    public partial class frmExamTrainingItemDetail : rRadDetailForm
    {
        public frmExamTrainingItemDetail(ExamTrainingItem item)
        {
            InitializeComponent();

            radPageView1.SelectedPage = radPageViewPage1;

            rCmbType.DataSource = typeof(ExamType).GetEnumDescriptions();
        ...
        {
            ExamTrainingItem item = GetProcessingObject<ExamTrainingItem>();
            ...
            ExamItem examItem = new ExamItem { ExamTrainingItem = item };
...
//            List<Lesson> lessons = major.GetLessons(item.Plan.Period).ToList();
        ...
        {
            ExamTrainingItem item = GetProcessingObject<ExamTrainingItem>();
            ExamForm form = new ExamForm { ExamTrainingItem = item };
        ...
        protected override void AfterBindToObject()
        {
            ...
            {
                if (rMessageBox.ShowQuestion("برای این آیتم، هیچ مواد آزمون تعریف نشده است. آیا مطمئن هستید ؟")
                    != DialogResult.Yes)
                {
                    CancelClosing();
                    return;
