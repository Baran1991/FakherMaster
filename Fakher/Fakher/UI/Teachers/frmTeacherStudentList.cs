...
{
    public partial class frmTeacherStudentList : rRadForm
    {
        public frmTeacherStudentList(Teacher teacher, EducationalPeriod period, Department department)
        {
            InitializeComponent();
            majorSelector1.Period = period;
            //majorSelector1.Major = DbContext.GetAll<Section>().Where(m => m.Teacher == teacher).Select(m => m.Major).Distinct();
            //lessonSelector1.Period = period;
            //sectionItemSelector1.Period = period;
