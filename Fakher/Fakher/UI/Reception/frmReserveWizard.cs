...
{
    public partial class frmReserveWizard : rRadForm
    {
        private static string mLastActionText;
        private bool mCustomChangeTabs;
        private ReserveWizardAction mAction;
...
//            else
        ...
        private void btnNext_Click(object sender, EventArgs e)
        {
            ...
            if (StepIndex == 0)
            {
                ...
                {
                    mAction = ReserveWizardAction.SectionReserve;
                    mLastActionText = rRadioButton1.Text;
                }
                if (rRadioButton2.ToggleState == ToggleState.On)
                {
                    mAction = ReserveWizardAction.ListReserve;
            ...
            if (StepIndex == 1)
            {
                ...
                financialDocumentControl1.Databind(new FinancialDocument(), FinancialHeading.ReserveSignup);
                GotoNextStep();
                return;
            }
        ...
        private void majorSelector2_SelectedChanged(object sender, EventArgs e)
        {
            ...
            if(major == null)
                return;
            if(mAction == ReserveWizardAction.ListReserve)
                rGridComboBoxReserveLst.DataSource = ReserveList.GetReserveList(Program.CurrentPeriod, major);
            if(mAction == ReserveWizardAction.SectionReserve)
                rGridComboBoxReserveLst.DataSource = ReserveList.GetSectionReserveList(Program.CurrentPeriod, major);
        }
    ...
    public enum ReserveWizardAction
    {
        SectionReserve,
        ListReserve
//        ReserveCancel
