...
namespace Fakher.Reports
{
    ...
    public partial class rptEnExerciseResult : Report, IConfigurableReport
    {
        private static Exam mExam;
        private static ExamSection mExamSection;

        public rptEnExerciseResult()
        {
            //
        ...
        private void textBox6_ItemDataBinding(object sender, EventArgs e)
        {
            if(mExamSection != null)
                textBox6.Value = "Exercise Result of " + mExam.Name + " " + mExamSection.SectionItem.Section.EnglishName + " [" + mExam.Date.ToShortDateString() + "] [" + mExam.QuestionCount + " Questions]";
            else
                textBox6.Value = "Exercise Result of " + mExam.Name + " [" + mExam.Date.ToShortDateString() + "] [" + mExam.QuestionCount + " Questions]";
        }

        public static float GetTotalMark(ExerciseParticipate examParticipate)
        {
            return examParticipate.CalculateMark();
        }

        public static float GetItemMark(ExerciseParticipate examParticipate, int examItemId)
        {
        ...
        public static int rptEnExamResult_GetRank(ExerciseParticipate participate)
        {
        ...
        private static IQueryable<float> GetMarks(ExamItem item)
        {
            ...
            if (mExamSection != null)
            {
                query = from participate in mExam.GetExerciseParticipates().ToList()
        ...
        private void AddAggregateColumns()
        {
            ...
            foreach (ExamItem examItem in mExam.Items)
            {
                ...
                txt1.Culture = new CultureInfo("en-us");
...
//                left += txt1.Size.Width.Value;
                ...
                txt2.Culture = new CultureInfo("en-us");
...
//                left += txt2.Size.Width.Value;
                ...
                txt3.Culture = new CultureInfo("en-us");
        ...
        {
            txtTotalMark.Value = "Total (" + mExam.EvaluationItem.Value + ")";
        }
        ...
        {
            get { return "گزارش نتایج تمرین (انگلیسی)"; }
        }
        ...
        public void PrepareDataset(IReportParameterForm frm)
        {
            ...
            if (frm.ExamSection != null)
            {
                ...
                //frm.Exam.CheckExerciseMarks(frm.ExamSection);
                IQueryable<ExerciseParticipate> participates = frm.Exam.GetEnglishOrderedExerciseParticipates(frm.ExamSection);
                //frm.Exam.EnsurePreCalculation(participates.ToList());
                DataSet = participates;
            }
            else
            {
                //frm.Exam.CheckAllExerciseMarks();
